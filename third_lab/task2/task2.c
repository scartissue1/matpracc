...
double scalar_norm(Vector vec, Vector * a_arg);
...
status_codes max_norms(Vector *** result, size_t * limit_norm_result_size, size_t * gelder_norm_result_size, size_t * scalar_norm_result_size, size_t dimension, 
                    double (*limit_norm)(Vector vec), double (*gelder_norm)(Vector vec, int p_arg), int p_arg_for_gelder, double (*scalar_norm)(Vector vec, Vector * matrix), size_t a_arg_scalar_size, Vector a_arg_for_skalar[], int quantity, ...) {
    if (p_arg_for_gelder < 1) return INVALID_PARAMETER;
    if (a_arg_scalar_size != a_arg_for_skalar[0].size) return INVALID_PARAMETER;
    ...
    for (int i = 0; i < quantity; i++) {
        ...
        double new_vector_scalar_norm = scalar_norm(new_vector, a_arg_for_skalar);
...
int main(int argc, char * argv[]) {
    ...
    Vector a_arg[3];
    size_t a_arg_size = 3;
    init_vector(&a_arg[0], 3, 1.0, 0.0, 0.0);
    init_vector(&a_arg[1], 3, 0.0, 1.0, 0.0);
    init_vector(&a_arg[2], 3, 0.0, 0.0, 1.0);

    switch (max_norms(&norm_maximums, &limit_norm_size, &gelder_norm_size, &scalar_norm_size, 3, limit_norm, gelder_norm, 3, scalar_norm, 3, a_arg, 3, first, second, third)) {
        case OK:
...
status_codes mult(Vector * result, Vector vec, Vector * matrix) {
    if (init_vector(result, vec.size) == NO_MEMORY) return NO_MEMORY;
    for (int i = 0; i < vec.size; i++) {
        result->coords[i] = 0;
        for (int j = 0; j < vec.size; j++) {
            result->coords[i] += matrix[j].coords[i] * vec.coords[i];
        }
    }
    return OK;
...
double scalar_norm(Vector vec, Vector * a_arg) {
    ...
    for (int i = 0; i < vec.size; i++) {
        Vector tmp;
        if (mult(&tmp, vec, a_arg) == NO_MEMORY) return -1;
        result += tmp.coords[i] * vec.coords[i];
    }
    result = sqrt(result);
    return result;
}
